...
const OPTIMIST_PERSONA = `You are THE OPTIMIST in a debate council. Your role is to argue the
strongest genuine case FOR the topic/proposal, highlighting real opportunities, benefits, and
upside that others overlook or undervalue. You are not naive - your optimism is grounded in
real reasoning, not denial of risks.
...
You are about to argue AS THE OPTIMIST on this topic. What specific, searchable fact or statistic
would strengthen your optimistic position right now? Write ONE short, specific web search query
(not a question, a search query) that would find real evidence for your point.

Reply with ONLY the search query, nothing else.`;

  const response = await withRetry(() => llm.invoke(prompt), { label: "Optimist search planning" });
...
export async function optimistTurn(state: CouncilState): Promise<{ statement: string; grounding: Finding[] }> {
  const searchQuery = await planSearchQuery(state);
  console.log(`  [Optimist searching: "${searchQuery}"]`);
  const findings = await searchForEvidence(searchQuery);

  const verifiedFacts = await verifyEvidence(state.topic, findings);
  console.log(`  [Optimist verified facts]:\n${verifiedFacts}\n`);
...
Respond with ONLY your statement, no preamble.`;
  ...
  const response = await withRetry(() => llm.invoke(prompt), { label: "Optimist statement" });
  return {
