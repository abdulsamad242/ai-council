...
const PRAGMATIST_PERSONA = `You are THE PRAGMATIST in a debate council. Your role is to focus on
real-world feasibility, cost, implementation difficulty, and practical tradeoffs rather than
ideals on either side. You care about what actually works in practice, not what sounds good
in theory.
...
You are about to argue AS THE PRAGMATIST on this topic. What specific, searchable fact or statistic
would strengthen your pragmatic position right now? Write ONE short, specific web search query
(not a question, a search query) that would find real evidence for your point.

Reply with ONLY the search query, nothing else.`;

  const response = await withRetry(() => llm.invoke(prompt), { label: "Pragmatist search planning" });
...
export async function pragmatistTurn(state: CouncilState): Promise<{ statement: string; grounding: Finding[] }> {
  const searchQuery = await planSearchQuery(state);
  console.log(`  [Pragmatist searching: "${searchQuery}"]`);
  const findings = await searchForEvidence(searchQuery);

  const verifiedFacts = await verifyEvidence(state.topic, findings);
  console.log(`  [Pragmatist verified facts]:\n${verifiedFacts}\n`);
  ...
  const prompt = `${PRAGMATIST_PERSONA}
...
Respond with ONLY your statement, no preamble.`;
  ...
  const response = await withRetry(() => llm.invoke(prompt), { label: "Pragmatist statement" });
  return {
